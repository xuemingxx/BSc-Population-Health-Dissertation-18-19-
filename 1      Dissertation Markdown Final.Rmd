...
# Multiple imputation
hse.mk60.mi <- aregImpute(~ sex + tenureb + origin + hhsize + addnum + imd + birthwt + frtpor + vegpor + gor + year
                          + aggr, data = hse.mk60, n.impute = 5, nk=0)
...
reg <- glm(hyper ~ sex + tenureb + origin + hhsize + addnum + imd + birthwt + frtpor + vegpor + gor + aggr, 
           family=binomial(link="logit"), data=hse.mk85)
hse.mk85$fit.value <- fitted.values(reg)

hse.mk85$imd <- hse.mk60.mi.r$imd
hse.mk85$imd <- ifelse(hse.mk85$imd < 3, 1, 0)
range(hse.mk85$imd)

# Propensity Scores Matching
hse.mk85$matching.vars <- cbind(hse.mk85$fit.value)
psm <- Match(Y=hse.mk85$hyper, Tr=hse.mk85$imd, X=hse.mk85$matching.vars, Weight = 2,version = "fast")
summary.Match(psm)

# Balance test
MatchBalance(imd ~ sex + tenureb + origin + hhsize + addnum + birthwt + frtpor + vegpor + gor + aggr, 
             match.out=psm, data=hse.mk60)
