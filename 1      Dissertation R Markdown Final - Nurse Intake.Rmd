...
#         Children are filtered from the datasets by "ag215g2", which is further converted to three age groups
...
```{r}
############      Data Preparation  --  Convert the inapplicable cases to NAs & Keep only the valid BP measurements     #######

#       As some of the observations are recorded as negative number or huge number in inapplicable cases in the datasets
...
# tenureb, origin, birthwt, porftvg, & six BP measurements has negative values (inapplicable cases)
# birthwt, porftvg has NA values originally which require to be imputed later.


#       Covert the inapplicable cases to NAs
...
#   Calculate .95 quantiles of the systolic and diastolic bp for each of the three age groups, 
...
quantile(hse.mk20$sys.hyper1, .95, na.rm = T)
hse.mk20$sys.hyper1.mk <- NA
hse.mk20$sys.hyper1.mk <- ifelse(hse.mk20$sys.hyper1 >= quantile(hse.mk20$sys.hyper1, .95, na.rm = T)[1], 1, 0)
...
quantile(hse.mk20$dia.hyper1, .95, na.rm = T)
hse.mk20$dia.hyper1.mk <- NA
hse.mk20$dia.hyper1.mk <- ifelse(hse.mk20$dia.hyper1 >= quantile(hse.mk20$dia.hyper1, .95, na.rm = T)[1], 1, 0)
...
ggplot(data = prop, mapping = aes(x = year, y = number, group = group)) + 
  ...
  labs(x="Number of Hypertensive Children",y="Years",title = "Number of Hypertensive Children across Years") +
...
# The first dimension explains 44% of the inertia. 
# The 2nd dimension explains a further 21.4%. 
...
# Covert the ordinal scale of treatment variable (imd) to binary scale for computing the propensity scores
hse.mk90 <- hse.mk60
...
summary.Match(psm)
#    1  --   Estimate effect of deprivation on hypertention (insignificant) 
# Balance test
...
#  Regression with a matched dataset  --  this time using ordinal scale of treatment (imd)
#  Create a new dataset after matching 
hse.mk.FX <- rbind(hse.mk60[psm$index.control,],hse.mk60[psm$index.treated,])

summary(lm(hyper ~ factor(imd),data=hse.mk.FX))
#    2  --   Balance our datasets for more accurate measure & conduct factor regression analysis
#            Estimating deprivation on a ordinal scales (significant) 
#       This shows that regression with a matched dataset is a good alternative to matching

summary(lm(hyper ~ factor(imd) + sex + tenureb + origin + hhsize + addnum + birthwt + porftvg + gor + aggr + year + sysavg + diaavg
           ,data=hse.mk.FX))
#    3  --   However, if we control other variables, deprivation is not significant predictors 
