...
#       Looking into the distribution of measurements
...
length(which(hse.mk20$porftvg < 0))
...
# The first dimension explains 41% of the inertia. 
...
hse.mk60.mi <- aregImpute(~ hyper + sex + tenureb + origin + hhsize + addnum + imd + birthwt + 
                            porftvg + gor + aggr + year, data = hse.mk60, n.impute = 50, nk=0)
...
hse.mk85$porftvg <- hse.mk60.mi.r$porftvg

hse.mk85$imd <- ifelse(hse.mk85$imd == 1, 1, 
                       ifelse(hse.mk85$imd == 5, 0, NA))
table(hse.mk85$imd)

hse.mk85.pc <- -c(which(is.na(hse.mk85$imd)))
hse.mk85 <- hse.mk85[hse.mk85.pc,]

# Compute the Propensity scores
reg <- glm(imd ~ sex + tenureb + origin + hhsize + addnum + birthwt + porftvg + gor + aggr + year, 
